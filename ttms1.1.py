    ...
    try:
        ...
        required_sheets = {
            ...
            "Orders": ["OrderID", "OrderName", "CustomerName", "Contact", "Pickup", "Destination", "Region",
                      "Distance", "Status", "Weight(KG)", "GST", "TotalAmount", "PaidAmount", "AmountStatus", "RemainingAmount"],
...
def admin_dashboard(user_type="Admin", login_window=None):
    ...
    buttons = [
        ...
        ("User Management", "icons/users.png",open_user_managemenr, 2, 1)
    ]
...
def driver_management_gui(user_role=""):
    ...
    form_fields = [
        ...
        [("Driver ID:", id_entry := ttk.Entry(form_frame)),
         ...
         ("License Expiry:", license_entry := ttk.Entry(form_frame)),
        ...
        [("Salary:", salary_entry := ttk.Entry(form_frame)),
         ...
         ("Joining Date:", joining_date_entry := ttk.Entry(form_frame)),
...
def truck_management_gui(user_role=""):
    ...
    form_fields = [
        ...
        ("Maintenance Schedule:", maintenance_entry := ttk.Entry(truck_info_frame)),
    ...
    maintenance_fields = [
        ...
        ("Date:", maintenance_date_entry := ttk.Entry(maintenance_frame))
    ]
    ...
    fuel_fields = [
        ...
        ("Date:", fuel_date_entry := ttk.Entry(fuel_frame))
    ]
...
def order_management_gui(user_role=""):
    ...
    # Update the add_order function:

    # Update the order_columns to match Excel layout:
    ...
    def clear_form():
        for widget in (order_id_entry,order_name_entry, customer_entry, contact_entry,
...
def dispatch_management_gui(user_role=""):
    ...
    ttk.Button(header, text="← Back", command=go_back).pack(side=tk.LEFT)
    ttk.Label(header, text="Dispatch Management System", font=("Helvetica", 16, "bold")).pack(side=tk.LEFT, padx=20)
    ...
    fields = [
        ...
        ("Estimated Delivery Time", estimated_entry := ttk.Entry(form_frame, width=30))
    ]
    ...
    def create_scrolled_tree(parent, columns, height=5):
    ...
    driver_tree, driver_container = create_scrolled_tree(drivers_frame, ("ID", "Name", "Status"))
    ...
    truck_tree, truck_container = create_scrolled_tree(trucks_frame, ("ID", "Model", "Status", "Capacity", "Permit"))
    ...
    dispatch_table, dispatch_container = create_scrolled_tree(dispatch_frame, dispatch_columns, height=10)
    ...
    order_tree, order_container = create_scrolled_tree(orders_frame, order_columns, height=10)
...
def accounts_management_gui(user_role=""):
    ...
    ttk.Button(header_frame, text="← Back", command=go_back).pack(side=tk.LEFT)
    ttk.Label(header_frame, text="Accounts & Financial Management", font=("Helvetica", 16, "bold")).pack(side=tk.LEFT, padx=20)
    payments_label = ttk.Label(header_frame, text="Total Payments: /-0.00", font=("Helvetica", 10, "bold"))
    payments_label.pack(side=tk.LEFT, padx=40, pady=5)

    expenses_label = ttk.Label(header_frame, text="Total Expenses: /-0.00", font=("Helvetica", 10, "bold"))
    expenses_label.pack(side=tk.LEFT, padx=20, pady=5)

    balance_label = ttk.Label(header_frame, text="Balance: /-0.00", font=("Helvetica", 10, "bold"))
    balance_label.pack(side=tk.LEFT, padx=20, pady=5)

    main_container = ttk.Frame(root)
    main_container.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

    forms_frame = ttk.Frame(main_container)
    forms_frame.pack(side=tk.LEFT, fill=tk.BOTH, padx=5, pady=5)

    expense_frame = ttk.LabelFrame(forms_frame, text="Record Expense")
    expense_frame.pack(fill=tk.X, pady=5, padx=5)
    ...
    ttk.Label(expense_frame, text="Payment Type:").grid(row=2, column=0, padx=5, pady=5)
    ...
    payment_frame.pack(fill=tk.X, pady=5, padx=5)
    ...
    invoice_frame.pack(fill=tk.X, pady=5, padx=5)
    ...
    # totals_frame = ttk.LabelFrame(forms_frame, text="Financial Summary")
    # totals_frame.pack(fill=tk.X, padx=5, pady=5)
    #
    # payments_label = ttk.Label(totals_frame, text="Total Payments: $0.00", font=("Helvetica", 10, "bold"))
    # payments_label.grid(row=0, column=0, padx=5, pady=5)
    #
    # expenses_label = ttk.Label(totals_frame, text="Total Expenses: $0.00", font=("Helvetica", 10, "bold"))
    # expenses_label.grid(row=1, column=0, padx=5, pady=5)
    #
    # balance_label = ttk.Label(totals_frame, text="Balance: $0.00", font=("Helvetica", 10, "bold"))
    ...
    transaction_table = ttk.Treeview(table_frame, columns=columns, show="headings", yscrollcommand=scrollbar.set)
    for col in columns:
        transaction_table.heading(col, text=col)
        transaction_table.column(col, width=120)

    transaction_table.pack(fill=tk.BOTH, expand=True)
    scrollbar.config(command=transaction_table.yview)
    # Add this code after the table_frame setup and before load_data_into_table():
    # totals_frame = ttk.LabelFrame(main_container, text="Financial Summary")
    # totals_frame.pack(side=tk.RIGHT, fill=tk.X, padx=5, pady=5)
    #
    # payments_label = ttk.Label(totals_frame, text="Total Payments: $0.00", font=("Helvetica", 10, "bold"))
    # payments_label.pack(side=tk.LEFT, padx=20, pady=5)
    #
    # expenses_label = ttk.Label(totals_frame, text="Total Expenses: $0.00", font=("Helvetica", 10, "bold"))
    # expenses_label.pack(side=tk.LEFT, padx=20, pady=5)
    #
    # balance_label = ttk.Label(totals_frame, text="Balance: $0.00", font=("Helvetica", 10, "bold"))
    # balance_label.pack(side=tk.LEFT, padx=20, pady=5)
    # Add these functions after the load_financial_data function:
    refresh_data()
    root.mainloop()
...
def reports_analytics_gui(user_role=""):
    ...
    header_frame.grid_columnconfigure(1, weight=1)  # Make the label expand

    ttk.Button(header_frame, text="← Back", command=go_back).grid(row=0, column=0, padx=5)
    ttk.Label(header_frame, text="Reports & Analytics",
              font=("Helvetica", 16, "bold")).grid(row=0, column=1, sticky="w", padx=20)
...
def user_management_gui():
    ...
    header.pack(fill=tk.X, pady=10)
    ttk.Button(header, text="← Back", command=go_back).pack(side=tk.LEFT)
    ttk.Label(header, text="User Management System", font=("Helvetica", 16, "bold")).pack(side=tk.LEFT, padx=20)
