...
def dispatch_management_gui(user_role=""):
    ...
    # Function to validate estimated delivery time
    ...
    # Function to check if driver is compliant (license expiry)
    def is_driver_compliant(driver_id):
        try:
            ...
            for row in sheet.iter_rows(min_row=2, values_only=True):
                if row[0] == driver_id:
                    expiry_date = row[5]  # Assuming column 5 is license expiry
        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2):
                if str(row[0].value) == str(dispatch_id):
                    row[4].value = new_status  # Update dispatch status
                    # Update corresponding order status
                    update_order_status(dispatch_id, new_status)
                    # Update progress bar
                    update_progress_bar(new_status)
                    break
            workbook.save(WORKBOOK_PATH)
        except Exception as e:
            messagebox.showerror("Error", f"Failed to update dispatch status: {e}")
            log_error(f"Failed to update dispatch status: {e}")
            ...
            try:
                ...
                for row_idx, row in enumerate(sheet.iter_rows(min_row=2), start=2):
                    if str(row[0].value) == str(dispatch_id):
                        ...
                        # Reset associated order status
                        update_order_status(dispatch_id, "Pending")
                        # Reset driver and truck status
    ...
    def update_progress_bar(status):
        ...
        progress_var.set(progress_values.get(status, 0))
    ...
    def update_selected_dispatch():
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to update dispatch: {e}")
            log_error(f"Failed to update dispatch: {e}")
    ...
    def get_available_drivers():
        try:
            ...
            available_drivers = []
            for row in sheet.iter_rows(min_row=2, values_only=True):
                if row[11] == "Available":  # Column 11 is Trip Status
    ...
    def get_available_trucks():
        try:
            ...
            available_trucks = []
            for row in sheet.iter_rows(min_row=2, values_only=True):
                if row[2] == "Operational":  # Changed from 'Operational' to 'Available'
    ...
    def update_truck_status(truck_id, new_status):
        try:
            ...
            sheet = workbook["Trucks"]
            for row in sheet.iter_rows(min_row=2):
                if row[0].value == truck_id:
                    row[2].value = new_status  # Update truck status
                    break
            workbook.save(WORKBOOK_PATH)
        except Exception as e:
    ...
    def get_pending_orders():
        try:
            ...
            pending_orders = []
            for row in sheet.iter_rows(min_row=2, values_only=True):
                if row[6] == "Pending":  # Assuming column 6 is Status
        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2):
                if row[0].value == order_id:  # First column is Order ID
                    row[6].value = new_status  # Column 6 is Status
                    break
            workbook.save(WORKBOOK_PATH)
        except Exception as e:
        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=2):
                if str(row[0].value) == str(driver_id):
                    row[11].value = new_status  # Column 11 is Trip Status
                    if new_status == "On Trip":
                        # Make sure to handle None value and convert to integer
    ...
    def assign_dispatch():
        ...
        if not is_valid_estimated_time(dispatch_time, estimated_time):
            messagebox.showerror("Error", "Estimated delivery time cannot be earlier than dispatch time!")
            return
        ...
        if not is_driver_compliant(driver_id):
            messagebox.showerror("Error", "Selected driver has expired license!")
            return
        ...
        update_truck_status(dispatch_data[2], "In Use")  # Update truck status
        load_data_into_table()
        refresh_combos()
    ...
    # Form
    form_frame = ttk.LabelFrame(root, text="Dispatch Assignment")
    form_frame.pack(fill=tk.X, padx=10, pady=10)

    # Order, Driver, Truck selection
    order_combo = ttk.Combobox(form_frame, values=get_pending_orders(), width=30)
    order_combo.grid(row=0, column=1, padx=10, pady=5)
    driver_combo = ttk.Combobox(form_frame, values=get_available_drivers(), width=30)
    driver_combo.grid(row=1, column=1, padx=10, pady=5)
    truck_combo = ttk.Combobox(form_frame, values=get_available_trucks(), width=30)
    truck_combo.grid(row=2, column=1, padx=10, pady=5)
    estimated_entry = ttk.Entry(form_frame, width=30)
    estimated_entry.grid(row=3, column=1, padx=10, pady=5)

    ttk.Label(form_frame, text="Order").grid(row=0, column=0, padx=10, pady=5)
    ttk.Label(form_frame, text="Driver").grid(row=1, column=0, padx=10, pady=5)
    ttk.Label(form_frame, text="Truck").grid(row=2, column=0, padx=10, pady=5)
    ttk.Label(form_frame, text="Estimated Delivery Time").grid(row=3, column=0, padx=10, pady=5)

    ttk.Button(form_frame, text="Assign Dispatch", command=assign_dispatch).grid(row=4, column=0, columnspan=2, pady=20)

    # Dispatch table
    table_frame = ttk.Frame(root)
    table_frame.pack(fill=tk.BOTH, padx=10, pady=10)

    columns = ("Order ID", "Driver", "Truck", "Dispatch Time", "Status", "Estimated Delivery Time")
    dispatch_table = ttk.Treeview(table_frame, columns=columns, show="headings")
    dispatch_table.pack(fill=tk.BOTH, expand=True)

    for col in columns:
        dispatch_table.heading(col, text=col)
    # Progress Bar
    progress_frame = ttk.LabelFrame(root, text="Dispatch Progress")
    ...
    update_button = ttk.Button(
        button_frame,
        text="Update Status",
        command=update_selected_dispatch
    )
