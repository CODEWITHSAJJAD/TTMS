...
def driver_management_gui(user_role=""):
    ...
    def generate_salary_report(return_data=False):
        ...
        try:
            ...
            # Header
            ttk.Label(main_frame, text=f"Monthly Salary Report ({current_month})",
            ...
            ttk.Label(summary_frame, text=f"Total Payments: {len(monthly_data)}",
                      font=("Helvetica", 10)).pack()
            ttk.Label(summary_frame, text=f"Total Amount: /-{sum(payment[1] for payment in monthly_data)}",
    ...
    def track_driver_status():
        ...
        try:
            ...
            # Header
            ttk.Label(main_frame, text="Driver Status Report",
                      font=("Helvetica", 14, "bold")).pack(pady=10)
            ...
            tree = ttk.Treeview(details_frame, columns=("ID", "Name", "Available", "Trip", "Off"),
            ...
            for driver_id, stats in driver_stats.items():
                tree.insert("", tk.END, values=(driver_id, stats['name'],
                                                stats['available_days'], stats['trip_days'],
                                                stats['off_days']))
    ...
    def manage_driver_leave():
        ...
        try:
            ...
            start_date = DateEntry(form_frame, width=19, background='darkblue',
            ...
            end_date = DateEntry(form_frame, width=19, background='darkblue',
                                 foreground='white', borderwidth=2, date_pattern='yyyy-mm-dd')
            ...
            ttk.Button(form_frame, text="Submit Request", command=submit_leave_request).grid(
    ...
    def export_reports(report_type):
        ...
        try:
            if report_type == "salary":
                ...
                filename = f"salary_report_{datetime.now().strftime('%Y%m')}.xlsx"
            elif report_type == "status":
                df = pd.DataFrame(track_driver_status(return_data=True))
                filename = f"status_report_{datetime.now().strftime('%Y%m%d')}.xlsx"
    ...
    def calculate_driver_performance():
        ...
        try:
            ...
            tree = ttk.Treeview(frame, columns=("ID", "Name", "Trips", "On-Time", "Score"),
                                show="headings")
    ...
    # Define form fields in 3 columns
    form_fields = [
        # First column
    ...
    # Create sub-frames for each row (you can adjust buttons_per_row as needed)
    buttons_per_row = 4
    buttons = [
        ...
        try:
            ...
            if filename:
                with open(filename, 'w', newline='') as csvfile:
                    ...
                    writer.writerow(["ID", "Model", "Status", "Permit",
                                     "Weight Capacity(KG)", "Maintenance Schedule", "Odometer"])
...
def order_management_gui(user_role=""):
    ...
    def export_to_pdf():
        try:
            ...
            filename = f"ordersdata_{timestamp}.pdf"
    ...
    def export_to_excel():
        ...
        try:
            ...
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            filename = f"ordersdata_{timestamp}.xlsx"
    ...
    def calculate_statistics():
        ...
        try:
            ...
            ttk.Button(button_frame, text="Refresh",
                       command=lambda: calculate_statistics()).pack(side=tk.LEFT, padx=5)

            # Modified export button
            ttk.Button(button_frame, text="Export",
                       command=lambda: export_statistics(metrics, export_type.get())).pack(side=tk.LEFT, padx=5)

            # Add print button
            ttk.Button(button_frame, text="Print",
                       command=lambda: print_statistics(stats_frame)).pack(side=tk.LEFT, padx=5)
    ...
    def export_statistics(metrics, format_type):
        ...
        try:
            ...
            filename = os.path.join(export_dir, f"statistics_{timestamp}.{format_type}")
